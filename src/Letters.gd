...
func button_pressed(buttonid):
	if(GLOBAL.get_busy_state() == false):
		...
		get_node("SamplePlayer2D").play(GLOBAL.get_current_soud(buttonid))
...
func draw_new_buttons_set():
	...
	var i=0
	while(i < GLOBAL.get_itens_lenght()):
		get_node("LettersPanel/ButtonsLayer/btn"+str(i)).set_normal_texture(load("res://drawable/l_" + GLOBAL.get_current_letter(i) + ".png"))
		get_node("LettersPanel/ButtonsLayer/btn"+str(i)).set_pressed_texture(load("res://drawable/pressed/l_" + GLOBAL.get_current_letter(i) + ".png"))
		i=i+1
